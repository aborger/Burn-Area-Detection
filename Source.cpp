...
int main(int argc, char* argv[]) {
	...
	// Checks for mode
	cout << "Enter (1) to test for burn areas!\nEnter (2) to count tree stands!" << endl;
	...
	do
	{
		...
		cin >> burn_or_tree;
		if (cin.fail() || burn_or_tree < 1 || burn_or_tree > 2) {
	...
	if (burn_or_tree == 1) {
		...
		// outputs
		cout << "Program complete. Enter (1) to print results to console.\nEnter (2) to output results to a file (Recommended for Bitterroot Nation Forest File)." << endl;
		...
		if (choice == 1) {
			...
			for (int a = 0; a < formattedList.getListSize(); a++) {
				if (formattedList.getAt(a).numThisSize > 0)
					cout << formattedList.getAt(a).numThisSize << " clumps with " << formattedList.getAt(a).size << " tree cells\n";
			}
		}
		else if (choice == 2) {
			...
			for (int a = 0; a < formattedList.getListSize(); a++) {
				if (formattedList.getAt(a).numThisSize > 0)
					outFile << formattedList.getAt(a).numThisSize << " clumps with " << formattedList.getAt(a).size << " tree cells\n";
			}
		}
		else {
			// error
		...
		cout << "Error with mode choice" << endl;
	}
